...
export class PizzIntIndicator {
  ...
  private render(): void {
    ...
    const defconEl = this.element.querySelector(
      ".pizzint-defcon",
    ) as HTMLElement;
    ...
    const locationsEl = this.element.querySelector(
      ".pizzint-locations",
    ) as HTMLElement;
    const updatedEl = this.element.querySelector(
      ".pizzint-updated",
    ) as HTMLElement;
    ...
    locationsEl.innerHTML = this.status.locations
      .map((loc) => {
  ...
  private renderTensions(): void {
    const listEl = this.element.querySelector(
      ".pizzint-tensions-list",
    ) as HTMLElement;
    if (!listEl) return;

    listEl.innerHTML = this.tensions
      .map((t) => {
        const trendIcon =
  ...
  private getStatusLabel(loc: {
    ...
    if (loc.current_popularity >= 40)
      return `ELEVATED ${loc.current_popularity}%`;
    if (loc.current_popularity >= 15)
      return `NOMINAL ${loc.current_popularity}%`;
