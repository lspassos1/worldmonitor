...
export class TechEventsPanel extends Panel {
  ...
  protected render(): void {
    ...
    const mappableCount = upcomingConferences.filter(
  ...
  private getFilteredEvents(): TechEvent[] {
    ...
    switch (this.viewMode) {
      ...
      case "conferences":
        return this.events
          .filter(
            (e) => e.type === "conference" && new Date(e.startDate) >= now,
          )
          .slice(0, 30);
  ...
  private renderEvent(event: TechEvent): string {
    ...
    const isSoon =
      !isToday &&
      startDate <= new Date(now.getTime() + 2 * 24 * 60 * 60 * 1000); // Within 2 days
    const isThisWeek =
