...
export class CountryIntelModal {
  ...
  public show(
    ...
    `;
    this.headerEl
      .querySelector(".country-intel-share-btn")
  ...
  public updateMarkets(markets: PredictionMarket[]): void {
    ...
    if (markets.length === 0) {
      section.innerHTML =
        '<span class="intel-loading-text" style="opacity:0.5">No prediction markets found</span>';
      return;
    }
    ...
    const items = markets
      .map((m) => {
        ...
        const link = safeUrl
          ? ` <a href="${safeUrl}" target="_blank" rel="noopener" class="market-link">↗</a>`
          : "";
        return `
