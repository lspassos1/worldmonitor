...
export class MapComponent {
  ...
  private baseLayerGroup: d3.Selection<
    SVGGElement,
    unknown,
    null,
    undefined
  > | null = null;
  private dynamicLayerGroup: d3.Selection<
    SVGGElement,
    unknown,
    null,
    undefined
  ...
  private showLayerHelp(): void {
    ...
    popup.innerHTML =
      SITE_VARIANT === "tech" ? techHelpContent : fullHelpContent;
  ...
  private syncLayerButtons(): void {
    this.container
      .querySelectorAll<HTMLButtonElement>(".layer-toggle")
      .forEach((btn) => {
  ...
  private runHealthCheck(): void {
    ...
    if (
      ...
      this.countryFeatures.length > 0 &&
      countryCount === 0
    ) {
      console.warn(
        "[Map] Health check: Base layer missing countries, initiating recovery",
      );
      ...
      (e) => {
        ...
        if (e.ctrlKey) {
          ...
          this.state.zoom = Math.max(
            1,
            Math.min(10, this.state.zoom + zoomDelta),
          );
        } else {
  ...
  private async loadMapData(): Promise<void> {
    try {
      ...
      if (this.worldData) {
        ...
        this.countryFeatures =
          "features" in countries ? countries.features : [countries];
      }
      this.baseRendered = false;
      this.render();
  ...
  public render(): void {
    ...
    const existingBase = svgNode.querySelector(
      ".map-base",
    ) as SVGGElement | null;
    const existingDynamic = svgNode.querySelector(
      ".map-dynamic",
    ) as SVGGElement | null;

    // Recreate layer groups if missing or if d3 selections are stale
    const baseStale =
      !existingBase || this.baseLayerGroup?.node() !== existingBase;
    const dynamicStale =
      !existingDynamic || this.dynamicLayerGroup?.node() !== existingDynamic;

    if (baseStale || dynamicStale) {
      // Clear any orphaned groups and create fresh ones
      svgNode
        .querySelectorAll(".map-base, .map-dynamic")
        .forEach((el) => el.remove());
      this.baseLayerGroup = this.svg.append("g").attr("class", "map-base");
      this.dynamicLayerGroup = this.svg
        .append("g")
        .attr("class", "map-dynamic");
      this.baseRendered = false;
      console.warn(
    ...
    const countryCount = this.baseLayerGroup
      .node()!
      .querySelectorAll(".country").length;
    ...
    if (
      ...
      const verifyCount =
        this.baseLayerGroup?.node()?.querySelectorAll(".country").length ?? 0;
      if (verifyCount === 0) {
        console.error(
  ...
  private renderCables(projection: d3.GeoProjection): void {
    ...
    const cableGroup = this.dynamicLayerGroup
      .append("g")
      .attr("class", "cables");
    ...
    UNDERSEA_CABLES.forEach((cable) => {
      ...
      const advisoryClass = cableAdvisory
        ? `cable-${cableAdvisory.severity}`
        : "";
      const highlightClass = isHighlighted
        ? "asset-highlight asset-highlight-cable"
        : "";
  ...
  private renderPipelines(projection: d3.GeoProjection): void {
    if (!this.dynamicLayerGroup) return;
    const pipelineGroup = this.dynamicLayerGroup
      .append("g")
    ...
    PIPELINES.forEach((pipeline) => {
      ...
      path
        .append("title")
        .text(`${pipeline.name} (${pipeline.type.toUpperCase()})`);
  ...
  private renderConflicts(projection: d3.GeoProjection): void {
    if (!this.dynamicLayerGroup) return;
    const conflictGroup = this.dynamicLayerGroup
      .append("g")
  ...
  private clusterMarkers<T extends { lat: number; lon: number }>(
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (let j = i + 1; j < items.length; j++) {
        ...
        if (
          !otherPos ||
          !Number.isFinite(otherPos[0]) ||
          !Number.isFinite(otherPos[1])
        )
          continue;
      ...
      const finalPos =
        centerPos &&
        Number.isFinite(centerPos[0]) &&
        Number.isFinite(centerPos[1])
          ? centerPos
          : pos;
  ...
  private renderOverlays(projection: d3.GeoProjection): void {
    ...
    if (this.state.layers.datacenters) {
      AI_DATA_CENTERS.filter(
        (dc) => (dc.chipCount || 0) >= MIN_GPU_COUNT,
      ).forEach((dc) => {
    ...
    if (this.state.layers.techHQs) {
      ...
      const clusterRadius =
        this.state.zoom >= 4 ? 15 : this.state.zoom >= 3 ? 25 : 40;
      ...
      clusters.forEach((cluster) => {
        ...
        if (isCluster) {
          ...
          const unicornCount = cluster.items.filter(
            (h) => h.type === "unicorn",
          ).length;
          const faangCount = cluster.items.filter(
            (h) => h.type === "faang",
          ).length;
          icon.textContent =
            faangCount > 0 ? "🏛️" : unicornCount > 0 ? "🦄" : "🏢";
          ...
          icon.textContent =
            primaryItem.type === "faang"
              ? "🏛️"
              : primaryItem.type === "unicorn"
                ? "🦄"
                : "🏢";
        }
        div.appendChild(icon);
        ...
        if (
          !isCluster &&
          (this.state.zoom >= 3 || primaryItem.type === "faang")
        ) {
          const label = document.createElement("div");
    ...
    if (this.state.layers.protests) {
      ...
      const clusterRadius =
        this.state.zoom >= 4 ? 12 : this.state.zoom >= 3 ? 20 : 35;
      const clusters = this.clusterMarkers(
        significantProtests,
        projection,
        clusterRadius,
      ...
      clusters.forEach((cluster) => {
        ...
        const hasHighSeverity = cluster.items.some(
          (e) => e.severity === "high",
        );
        ...
        icon.textContent = hasRiot
          ? "🔥"
          : primaryEvent.eventType === "strike"
            ? "✊"
            : "📢";
        div.appendChild(icon);
      ...
      this.militaryVessels.forEach((vessel) => {
        ...
        if (vessel.track && vessel.track.length > 1 && this.state.zoom >= 2) {
          ...
          if (points) {
            ...
            trackLine.setAttribute(
              "class",
              `military-vessel-track ${vessel.operator}`,
            );
            trackLine.setAttribute("fill", "none");
  ...
  private renderAisDensity(projection: d3.GeoProjection): void {
    if (!this.dynamicLayerGroup) return;
    const densityGroup = this.dynamicLayerGroup
      .append("g")
      .attr("class", "ais-density");
  ...
  public triggerPipelineClick(id: string): void {
    ...
    const height = this.container.clientHeight;
    const projection = this.getProjection(width, height);
    const midPoint = pipeline.points[
      Math.floor(pipeline.points.length / 2)
    ] as [number, number];
  ...
  public triggerCableClick(id: string): void {
    ...
    const height = this.container.clientHeight;
    const projection = this.getProjection(width, height);
    const midPoint = cable.points[Math.floor(cable.points.length / 2)] as [
      number,
      number,
    ];
    const pos = projection(midPoint);
  ...
  private updateZoomLayerVisibility(): void {
    const zoom = this.state.zoom;
    (
      Object.keys(MapComponent.LAYER_ZOOM_THRESHOLDS) as (keyof MapLayers)[]
    ).forEach((layer) => {
      const thresholds = MapComponent.LAYER_ZOOM_THRESHOLDS[layer];
  ...
  private updateLabelVisibility(zoom: number): void {
    ...
    labels.forEach((label) => {
      ...
      const parent = el.closest(
        ".hotspot, .earthquake-marker, .weather-marker, .apt-marker",
      );
    ...
    labelRects.forEach(({ el, rect, priority }) => {
      const overlaps = visibleRects.some((vr) => {
        const dx = Math.abs(
          rect.left + rect.width / 2 - (vr.left + vr.width / 2),
        );
        const dy = Math.abs(
          rect.top + rect.height / 2 - (vr.top + vr.height / 2),
        );
        return (
  ...
  private ensureBaseLayerIntact(): void {
    ...
    if (domBaseGroup && selectionNode !== domBaseGroup) {
      console.warn(
        "[Map] Stale base layer selection detected, forcing full rebuild",
      );
      this.baseRendered = false;
      this.render();
      return;
    ...
    if (
      ...
      console.warn(
        "[Map] Base layer missing countries, triggering recovery render",
      );
      this.baseRendered = false;
      this.render();
    }
  ...
  public setEarthquakes(earthquakes: Earthquake[]): void {
    console.log(
      "[Map] setEarthquakes called with",
      earthquakes.length,
      "earthquakes",
    );
  ...
  private getCableAdvisory(cableId: string): CableAdvisory | undefined {
    const advisories = this.cableAdvisories.filter(
      (advisory) => advisory.cableId === cableId,
    );
  ...
  private getCableName(cableId: string): string {
    return (
      UNDERSEA_CABLES.find((cable) => cable.id === cableId)?.name || cableId
    );
  }
