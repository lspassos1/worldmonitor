...
export class MapPopup {
  ...
  private renderConflictPopup(conflict: ConflictZone): string {
    ...
    const severityLabel = escapeHtml(
      conflict.intensity?.toUpperCase() || "UNKNOWN",
    );
  ...
  private renderAisPopup(event: AisDisruptionEvent): string {
    ...
    const typeLabel =
      event.type === "gap_spike" ? "AIS GAP SPIKE" : "CHOKEPOINT CONGESTION";
    const changeLabel = event.type === "gap_spike" ? "DARKENING" : "DENSITY";
    const countLabel =
      event.type === "gap_spike" ? "DARK SHIPS" : "VESSEL COUNT";
    const countValue =
    ...
    const icon =
      event.eventType === "riot"
        ...
        : event.eventType === "strike"
          ...
          : "📢";
    const sourceLabel =
      event.sourceType === "acled" ? "ACLED (verified)" : "GDELT";
  ...
  private renderProtestClusterPopup(data: ProtestClusterData): string {
    ...
    const listItems = sortedItems
      ...
      .map((event) => {
        ...
        const dateStr = event.time.toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
        });
    ...
    const moreCount =
      remainingCount > 0
        ? `<li class="cluster-more">+${remainingCount} more events</li>`
        : "";
    const headerClass =
  ...
  private renderCablePopup(cable: UnderseaCable): string {
    ...
    const statusLabel = advisory
      ? advisory.severity === "fault"
        ? "FAULT"
        : "DEGRADED"
      : "ACTIVE";
    const statusBadge = advisory
      ? advisory.severity === "fault"
        ? "high"
        : "elevated"
      : "low";
    ...
    const repairShipNote = repairShip
      ? escapeHtml(
          repairShip.note ||
            "Repair vessel tracking indicates active deployment toward fault site.",
        )
      : "";
  ...
  private renderRepairShipPopup(ship: RepairShip): string {
    ...
    const shipNote = escapeHtml(
      ship.note ||
        "Repair ship tracking indicates active deployment in support of undersea cable restoration.",
    );
  ...
  private renderMilitaryFlightPopup(flight: MilitaryFlight): string {
    ...
    const operatorLabel = escapeHtml(
      operatorLabels[flight.operator] || flight.operatorCountry || "Unknown",
    );
  ...
  private renderMilitaryVesselPopup(vessel: MilitaryVessel): string {
    ...
    const vesselOperator = escapeHtml(
      operatorLabels[vessel.operator] || vessel.operatorCountry || "Unknown",
    );
  ...
  private renderMilitaryVesselClusterPopup(
    ...
    const vesselSummary = cluster.vessels
      ...
      .map(
        (v) =>
          `<div class="cluster-vessel-item">${escapeHtml(v.name)} - ${escapeHtml(v.vesselType)}</div>`,
      )
      .join("");
    const moreVessels =
      cluster.vesselCount > 5
