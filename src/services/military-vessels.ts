 ...
 */
function analyzeMmsi(mmsi: string): {
 ...
 */
function matchKnownVessel(
  name: string,
): (typeof KNOWN_NAVAL_VESSELS)[number] | undefined {
 ...
 */
function getVesselTypeFromAis(
  shipType: number,
): MilitaryVesselType | undefined {
  // AIS ship type codes
  // 35 = Military ops
  // 50-59 = Special craft
  // 55 = Law enforcement
...
function getNearbyBase(lat: number, lon: number): string | undefined {
  for (const base of NAVAL_BASES) {
    const distance = Math.sqrt(
      Math.pow(lat - base.lat, 2) + Math.pow(lon - base.lon, 2),
    );
    if (distance <= 0.5) {
      // Within ~50km
      return base.name;
...
function processAisPosition(data: AisPositionData): void {
  ...
  const aisType = data.shipType
    ? getVesselTypeFromAis(data.shipType)
    : undefined;
  ...
  const vessel: MilitaryVessel = {
    ...
    confidence: knownVessel
      ? "high"
      : mmsiAnalysis.isPotentialMilitary
        ? "medium"
        : "low",
    isInteresting: Boolean(
      nearHotspot?.priority === "high" || isDark || nearChokepoint,
    ),
    note: isDark
  ...
  if (messageCount % 50 === 0) {
    console.log(
      `[Military Vessels] Tracking ${trackedVessels.size} military/gov vessels`,
    );
  }
}
...
function clusterVessels(vessels: MilitaryVessel[]): MilitaryVesselCluster[] {
  ...
  for (const hotspot of MILITARY_HOTSPOTS) {
    ...
    if (nearbyVessels.length >= 2) {
      ...
      const avgLat =
        nearbyVessels.reduce((sum, v) => sum + v.lat, 0) / nearbyVessels.length;
      const avgLon =
        nearbyVessels.reduce((sum, v) => sum + v.lon, 0) / nearbyVessels.length;
      ...
      let activityType: "exercise" | "deployment" | "transit" | "unknown" =
        "unknown";
...
export function initMilitaryVesselStream(): void {
  ...
  console.log(
    "[Military Vessels] Initializing tracking via shared AIS stream...",
  );
  ...
  return breaker.execute(
    async () => {
      ...
      if (vesselCache && Date.now() - vesselCache.timestamp < CACHE_TTL) {
        console.log(
          `[Military Vessels] Returning cached: ${vesselCache.data.length} vessels`,
        );
      ...
      if (!isTracking && isAisConfigured()) {
        console.log(
          "[Military Vessels] Initializing stream from fetchMilitaryVessels...",
        );
...
export function getVesselsNearLocation(
  ...
  for (const vessel of trackedVessels.values()) {
    const distance = Math.sqrt(
      Math.pow(vessel.lat - lat, 2) + Math.pow(vessel.lon - lon, 2),
    );
