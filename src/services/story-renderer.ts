...
export async function renderStoryToCanvas(
  ...
  // Trend + level badges
  const trendIcon =
    data.cii?.trend === "rising"
      ? "▲"
      : data.cii?.trend === "falling"
        ? "▼"
        : "●";
  ...
  if (data.cii?.components) {
    ...
    for (const comp of comps) {
      ...
      roundRect(
        ...
        cx,
        y + 8,
        (compBarW * Math.min(comp.val, 100)) / 100,
        8,
        4,
      );
      ctx.fill();
  ...
  if (data.convergence && data.convergence.score > 0) {
    ...
    const convColor =
      convScore >= 70 ? "#ef4444" : convScore >= 40 ? "#eab308" : "#22c55e";
    ...
    if (data.convergence.signalTypes.length > 0) {
      ...
      ctx.fillText(
        data.convergence.signalTypes.map(humanizeSignalType).join("  ·  "),
        PAD,
        y,
      );
    }
  ...
  if (data.news.length > 0 && y < FOOTER_Y - 200) {
    ...
    y += 36;
    const totalSources = data.news.reduce(
      (s, n) => s + (n.sourceCount || 1),
      0,
    );
  ...
  if (data.theater && y < FOOTER_Y - 200) {
    ...
    const acW = ctx.measureText(
      `✈ ${data.theater.totalAircraft} aircraft`,
    ).width;
  ...
  if (data.markets.length > 0 && y < FOOTER_Y - 150) {
    ...
    for (const m of data.markets.slice(0, 4)) {
      ...
      const pctStr = `${pct}%`;
      const pctColor =
        pct >= 70 ? "#ef4444" : pct >= 40 ? "#eab308" : "#22c55e";
  ...
  // ── THREAT BREAKDOWN ──
  const hasThreats =
    data.threats.critical + data.threats.high + data.threats.medium > 0;
