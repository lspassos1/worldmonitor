...
const ucdpBreaker = createCircuitBreaker<Map<string, UcdpConflictStatus>>({
  name: "UCDP Classifications",
});

export async function fetchUcdpClassifications(): Promise<
  Map<string, UcdpConflictStatus>
> {
  return ucdpBreaker.execute(async () => {
    ...
    for (const c of conflicts) {
      ...
      if (
        !existing ||
        c.year > existing.year ||
        (c.year === existing.year &&
          c.intensityLevel >
            (existing.intensity === "war"
              ? 2
              : existing.intensity === "minor"
                ? 1
                : 0))
      ) {
        byCountry.set(code, {
