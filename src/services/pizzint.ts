...
const DEFCON_THRESHOLDS: Array<{
  level: PizzIntDefconLevel;
  min: number;
  label: string;
}> = [
...
function calculateDefcon(
  ...
  activeSpikes: number,
): { level: PizzIntDefconLevel; label: string } {
...
export async function fetchPizzIntStatus(): Promise<PizzIntStatus> {
  return pizzintBreaker.execute(async () => {
    ...
    const locationsWithUsual = locations.filter(
      (l) => l.percentage_of_usual !== null && l.percentage_of_usual > 0,
    );
    const aggregateActivity =
      locationsWithUsual.length > 0
        ? Math.round(
            locationsWithUsual.reduce(
              (sum, l) => sum + Math.min(l.percentage_of_usual ?? 0, 100),
              0,
            ) / locationsWithUsual.length,
          )
        : openLocations.length > 0
          ? Math.round(
              openLocations.reduce((sum, l) => sum + l.current_popularity, 0) /
                openLocations.length,
            )
          : 0;

    const activeSpikes = locations.filter((l) => l.is_spike).length;
    const freshness = locations.some((l) => l.data_freshness === "fresh")
      ? "fresh"
      : "stale";
...
const TENSION_PAIRS = [
  {
    id: "usa_russia",
    countries: ["USA", "Russia"] as [string, string],
    label: "USA ↔ Russia",
    region: "europe",
  },
  {
    id: "russia_ukraine",
    countries: ["Russia", "Ukraine"] as [string, string],
    label: "Russia ↔ Ukraine",
    region: "europe",
  },
  {
    id: "usa_china",
    countries: ["USA", "China"] as [string, string],
    label: "USA ↔ China",
    region: "asia",
  },
  {
    id: "china_taiwan",
    countries: ["China", "Taiwan"] as [string, string],
    label: "China ↔ Taiwan",
    region: "asia",
  },
  {
    id: "usa_iran",
    countries: ["USA", "Iran"] as [string, string],
    label: "USA ↔ Iran",
    region: "middle_east",
  },
  {
    id: "usa_venezuela",
    countries: ["USA", "Venezuela"] as [string, string],
    label: "USA ↔ Venezuela",
    region: "americas",
  },
];
...
export async function fetchGdeltTensions(): Promise<GdeltTensionPair[]> {
  return gdeltBreaker.execute(async () => {
    ...
    return TENSION_PAIRS.map((pair) => {
      ...
      const olderAvg =
        older.length > 0
          ? older.reduce((s, d) => s + d.v, 0) / older.length
          : recentAvg;
