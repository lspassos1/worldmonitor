...
function generateDescription(
  ...
  if (score !== undefined && level) {
    parts.push(
      `${countryName} has an instability score of ${score}/100 (${level})`,
    );
  }
  ...
  const typeDescriptions: Record<string, string> = {
    ciianalysis:
      "Full intelligence analysis with military posture and prediction markets",
...
export function parseStoryParams(url: URL): StoryMeta | null {
  ...
  const safeType: StoryMeta["type"] = validTypes.includes(
