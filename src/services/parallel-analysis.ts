...
class ParallelAnalysisService {
  ...
  async analyzeHeadlines(clusters: ClusteredEvent[]): Promise<AnalysisReport> {
    ...
    if (mlWorker.isAvailable) {
      ...
      if (entities)
        console.log(
          `%c  → NER extracted ${entities.flat().length} entities`,
          "color: #888",
        );
      if (embeddings)
        console.log(
          `%c  → Embeddings computed for ${embeddings.length} headlines`,
          "color: #888",
        );
    } else {
      console.log(
        `%c⚠ ML not available, using keyword-only analysis`,
        "color: #f59e0b",
      );
    }
  ...
  private logReport(report: AnalysisReport, durationMs: number): void {
    console.log(
      `%c⏱ Analysis completed in ${durationMs.toFixed(0)}ms`,
      "color: #888",
    );

    console.log(
      "\n%c📊 TOP BY CONSENSUS (high confidence)",
    ...
    console.table(
      report.topByConsensus.slice(0, 5).map((h) => ({
        ...
        topPerspective: h.perspectives.sort((a, b) => b.score - a.score)[0]
          ?.name,
      })),
    );
    ...
    if (report.missedByKeywords.length > 0) {
      ...
      report.missedByKeywords.forEach((h) => {
        const keywordScore =
          h.perspectives.find((p) => p.name === "keywords")?.score ?? 0;
        ...
        mlScores.forEach((p) => {
          console.log(
            `      ${p.name}: ${(p.score * 100).toFixed(0)}% - ${p.reasoning}`,
          );
        });
      });
    }
