  ...
  return clusters
    .map((cluster) => {
      ...
      const locItems = cluster.filter(
        (i): i is NewsItemWithTier & { lat: number; lon: number } =>
      ...
      if (locItems.length > 0) {
        ...
        const best = Array.from(locCounts.values()).sort(
          (a, b) => b.count - a.count,
        )[0]!;
        clusterLat = best.lat;
        clusterLon = best.lon;
...
function pruneVelocityHistory(
  history: TopicVelocityPoint[],
  now: number,
): TopicVelocityPoint[] {
  return history.filter(
    (point) => now - point.timestamp <= TOPIC_BASELINE_WINDOW_MS,
  );
}
...
export function detectPipelineFlowDrops(
  ...
  for (const event of events) {
    ...
    const hasPipeline = titles.some((title) =>
      includesKeyword(title, PIPELINE_KEYWORDS),
    );
    const hasFlowDrop = titles.some((title) =>
      includesKeyword(title, FLOW_DROP_KEYWORDS),
    );
...
export function analyzeCorrelationsCore(
  ...
  const pipelineFlowSignals = detectPipelineFlowDrops(
    events,
    isRecentDuplicate,
    markSignalSeen,
  );
  ...
  const previousHistory =
    previousSnapshot?.topicVelocityHistory ??
    new Map<string, TopicVelocityPoint[]>();
  ...
  for (const topic of topicUniverse) {
    ...
    const updated = [
      ...prior,
      { timestamp: now, velocity: newsTopics.get(topic) ?? 0 },
    ];
  ...
  const currentSnapshot: StreamSnapshot = {
    ...
    predictionChanges: new Map(
      predictions.map((p) => [p.title.slice(0, 50), p.yesPrice]),
    ),
  ...
  for (const pred of predictions) {
    ...
    if (prev !== undefined) {
      ...
      if (shift >= PREDICTION_SHIFT_THRESHOLD) {
        ...
        const newsActivity = related.reduce(
          (sum, t) => sum + (newsTopics.get(t) ?? 0),
          0,
        );
        ...
        if (
          newsActivity < NEWS_VELOCITY_THRESHOLD &&
          !isRecentDuplicate(dedupeKey)
        ) {
          markSignalSeen(dedupeKey);
          signals.push({
            id: generateSignalId(),
  ...
  for (const [topic, velocity] of newsTopics) {
    ...
    const baselineHistory = pruneVelocityHistory(
      previousHistory.get(topic) ?? [],
      now,
    );
    ...
    if (!isRecentDuplicate(dedupeKey)) {
      ...
      signals.push({
        ...
        confidence: Math.min(
          0.9,
          0.45 + (multiplier > 0 ? multiplier / 8 : velocity / 18),
        ),
        timestamp: new Date(),
        data: {
  ...
  for (const market of markets) {
    ...
    const relatedNews = findNewsForMarketSymbol(
      market.symbol,
      newsEntityContexts,
    );
    ...
    if (relatedNews.length > 0) {
      ...
      if (!isRecentDuplicate(dedupeKey)) {
        ...
        signals.push({
          ...
          confidence: Math.min(
            0.9,
            0.5 + relatedNews.length * 0.1 + change / 20,
          ),
          timestamp: new Date(),
          data: {
      ...
      const oldRelatedNews = Array.from(newsTopics.entries())
        .filter(
          ([k]) =>
            market.name.toLowerCase().includes(k) ||
            k.includes(market.symbol.toLowerCase()),
        )
