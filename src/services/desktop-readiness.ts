...
export const DESKTOP_PARITY_FEATURES: DesktopParityFeature[] = [
  ...
  },
  {
    ...
    locality: "fully-local",
    fallback:
      "Keyword monitoring runs fully client-side on loaded news corpus.",
    priority: 1,
  },
  {
    id: "strategic-risk",
...
export function getNonParityFeatures(): DesktopParityFeature[] {
  return DESKTOP_PARITY_FEATURES.filter(
    (feature) => feature.locality !== "fully-local",
  );
}

export function getDesktopReadinessChecks(
  localBackendEnabled: boolean,
): DesktopReadinessCheck[] {
  const liveTrackingReady =
    isFeatureAvailable("aisRelay") || isFeatureAvailable("openskyRelay");

  return [
    {
      id: "startup",
      label: "Desktop startup + sidecar API health",
      ready: localBackendEnabled,
    },
    {
      id: "map",
      label: "Map rendering (local layers + static geo assets)",
      ready: true,
    },
    {
      id: "core-intel",
    ...
    },
    {
      ...
      ready: isFeatureAvailable("aiGroq") || isFeatureAvailable("aiOpenRouter"),
    },
...
export function getKeyBackedAvailabilitySummary(): {
  available: number;
  total: number;
} {
  const available = keyBackedFeatures.filter((featureId) =>
