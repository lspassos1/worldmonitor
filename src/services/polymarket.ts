...
async function polyFetch(
  ...
  if (directFetchWorks !== false) {
    try {
      ...
      console.log(
        "[Polymarket] Direct fetch blocked by Cloudflare, using proxy",
      );
    }
  }
  ...
  if (isDesktopRuntime()) {
    try {
      const body = await tryInvokeTauri<string>("fetch_polymarket", {
        path: endpoint,
        params: qs,
      });
      if (body) {
...
export async function fetchPredictions(): Promise<PredictionMarket[]> {
  return breaker.execute(async () => {
    ...
    const eventResults = await Promise.all(
      tags.map((tag) => fetchEventsByTag(tag, 20)),
    );
...
export async function fetchCountryMarkets(
  ...
  try {
    const eventResults = await Promise.all(
      uniqueTags.map((tag) => fetchEventsByTag(tag, 30)),
    );
    const seen = new Set<string>();
    const markets: PredictionMarket[] = [];
    ...
    for (const events of eventResults) {
      for (const event of events) {
        ...
        if (!eventTitleMatches) {
          const marketTitles = (event.markets ?? []).map((m) =>
        ...
        if (event.markets && event.markets.length > 0) {
          ...
          const candidates = eventTitleMatches
            ...
            : event.markets.filter((m) =>
                variants.some((v) =>
                  (m.question ?? "").toLowerCase().includes(v),
                ),
              );
