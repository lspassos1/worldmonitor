...
const acledBreaker = createCircuitBreaker<SocialUnrestEvent[]>({
  name: "ACLED Protests",
});
...
function findNearbyHotspots(
  ...
  return INTEL_HOTSPOTS.filter(
    (h) => haversineKm(lat, lon, h.lat, h.lon) < radiusKm,
  ).map((h) => h.name);
}

// ACLED event type mapping
function mapAcledEventType(
  eventType: string,
  subEventType: string,
): ProtestEventType {
    ...
    return events
      ...
      .map((e): SocialUnrestEvent => {
        ...
        return {
          ...
          title:
            e.notes?.slice(0, 200) || `${e.sub_event_type} in ${e.location}`,
          summary: e.notes,
  ...
  for (const event of events) {
    ...
    if (!existing) {
      ...
      if (event.sourceType === "acled" && existing.sourceType !== "acled") {
        ...
        existing.sources = [
          ...new Set([...existing.sources, ...event.sources]),
        ];
...
function sortEvents(events: SocialUnrestEvent[]): SocialUnrestEvent[] {
  const severityOrder: Record<ProtestSeverity, number> = {
    high: 0,
    medium: 1,
    low: 2,
  };
...
export async function fetchProtestEvents(): Promise<ProtestData> {
  ...
  console.log(
    `[Protests] Fetched ${acledEvents.length} ACLED, ${gdeltEvents.length} GDELT events`,
  );
...
export function getProtestStatus(): {
  acledConfigured: boolean | null;
