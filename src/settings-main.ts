  ...
  document.getElementById("okBtn")?.addEventListener("click", () => {
    void (async () => {
      try {
        ...
        console.log(
          "[settings] commit done, remaining pending:",
          panel.hasPendingChanges(),
        );
        if (errors.length > 0) {
          setActionStatus(
            `Saved verified keys. Failed: ${errors.join(", ")}`,
            "error",
          );
        } else {
...
function initDiagnostics(): void {
  ...
  verboseToggle?.addEventListener("change", async () => {
    try {
      ...
      setActionStatus(
        data.verboseMode
          ? "Verbose sidecar logging ON (saved)"
          : "Verbose sidecar logging OFF (saved)",
        "ok",
      );
    } catch {
      setActionStatus(
        "Could not reach sidecar to toggle verbose mode",
        "error",
      );
    }
  });
    ...
    try {
      ...
      if (entries.length === 0) {
        trafficLogEl.innerHTML =
          '<p class="diag-empty">No traffic recorded yet.</p>';
        return;
      }
      ...
      trafficLogEl.innerHTML =
        '<p class="diag-empty">Sidecar not reachable.</p>';
    }
  }
  ...
  clearBtn?.addEventListener("click", async () => {
    try {
      await fetch(`${SIDECAR_BASE}/api/local-traffic-log`, {
        method: "DELETE",
      });
    } catch {
      /* ignore */
    ...
    if (trafficLogEl)
      trafficLogEl.innerHTML = '<p class="diag-empty">Log cleared.</p>';
