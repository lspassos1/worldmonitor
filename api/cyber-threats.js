...
const URLHAUS_RECENT_URL = (limit) =>
...
function inferFeodoSeverity(record, malwareFamily) {
  ...
  const status = cleanString(
    record?.status || record?.c2_status || "",
    30,
  ).toLowerCase();
...
function inferUrlhausType(record, tags) {
  const threat = cleanString(
    record?.threat || record?.threat_type || "",
    40,
  ).toLowerCase();
  ...
  if (
    threat.includes("malware") ||
    threat.includes("payload") ||
    allTags.includes("malware")
  )
    return "malware_host";
...
function sanitizeThreat(threat) {
  ...
  const indicatorType = normalizeEnum(
  ...
  const type = normalizeEnum(
    threat?.type,
    ALLOWED_TYPES,
    source === "feodo" ? "c2_server" : "malicious_url",
  );
  ...
  return {
    id:
      cleanString(threat?.id, 255) || `${source}:${indicatorType}:${indicator}`,
    type,
    source,
    indicator,
    indicatorType,
...
function parseFeodoRecord(record, cutoffMs) {
  ...
  const statusRaw = cleanString(
    record?.status || record?.c2_status || "",
    30,
  ).toLowerCase();
  // Accept both online and recently-offline (still threat-relevant)
  if (statusRaw && statusRaw !== "online" && statusRaw !== "offline")
    return null;

  const firstSeen = toIsoDate(
    record?.first_seen || record?.first_seen_utc || record?.dateadded,
  );
  ...
  const sanitized = sanitizeThreat({
    ...
    severity:
      statusRaw === "online"
        ? inferFeodoSeverity(record, malwareFamily)
        : "medium",
    malwareFamily,
    tags: ["botnet", "c2", ...tags],
    firstSeen,
...
function parseUrlhausRecord(record, cutoffMs) {
  ...
  const statusRaw = cleanString(
    record?.url_status || record?.status || "",
    30,
  ).toLowerCase();
  ...
  const recordIp = cleanString(
    record?.host || record?.ip_address || record?.ip,
    80,
  ).toLowerCase();
  ...
  const firstSeen = toIsoDate(
    record?.dateadded || record?.firstseen || record?.first_seen,
  );
  const lastSeen = toIsoDate(
    record?.last_online || record?.last_seen || record?.dateadded,
  );
...
function parseC2IntelCsvLine(line) {
  ...
  const severity = /cobaltstrike|cobalt.strike|brute.?ratel/i.test(description)
    ? "high"
    : "medium";
...
export function __testDedupeThreats(threats) {
  ...
  for (const threat of Array.isArray(threats) ? threats : []) {
    ...
    const existingSeen = Date.parse(
      existing.lastSeen || existing.firstSeen || "1970-01-01T00:00:00.000Z",
    );
    const candidateSeen = Date.parse(
      sanitized.lastSeen || sanitized.firstSeen || "1970-01-01T00:00:00.000Z",
    );
...
async function fetchGeoIp(ip) {
  ...
  try {
    const primary = await fetchJsonWithTimeout(
      `https://ipinfo.io/${encodeURIComponent(ip)}/json`,
      {},
      GEO_PER_IP_TIMEOUT_MS,
    );
    if (primary.ok) {
...
async function hydrateThreatCoordinates(threats) {
  ...
  return threats.map((threat) => {
    ...
    const lookup = resolvedByIp.get(
      cleanString(threat.indicator, 80).toLowerCase(),
    );
    if (lookup) {
      return {
        ...threat,
    ...
    const parsed = records
      ...
      .sort((a, b) => {
        const aTime = Date.parse(
          a.lastSeen || a.firstSeen || "1970-01-01T00:00:00.000Z",
        );
        const bTime = Date.parse(
          b.lastSeen || b.firstSeen || "1970-01-01T00:00:00.000Z",
        );
        return bTime - aTime;
      })
      .slice(0, limit);
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ok: false,
        threats: [],
        reason: `otx_http_${response.status}`,
        enabled: true,
      };
    }
    ...
    for (const record of results) {
      ...
      const severity = tags.some((t) => /ransomware|apt|c2|botnet/.test(t))
        ? "high"
        : "medium";

      const sanitized = sanitizeThreat({
        id: `otx:${ip}`,
        type: tags.some((t) => /c2|botnet/.test(t))
          ? "c2_server"
          : "malware_host",
        source: "otx",
        indicator: ip,
    ...
    return {
      ok: false,
      threats: [],
      reason: `otx_error:${cleanString(toErrorMessage(error), 120)}`,
      enabled: true,
    };
  }
}
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ok: false,
        threats: [],
        reason: `abuseipdb_http_${response.status}`,
        enabled: true,
      };
    }
    ...
    for (const record of records) {
      ...
      const severity =
        score >= 95 ? "critical" : score >= 80 ? "high" : "medium";
    ...
    return {
      ok: false,
      threats: [],
      reason: `abuseipdb_error:${cleanString(toErrorMessage(error), 120)}`,
      enabled: true,
    };
  }
}
...
export default async function handler(req) {
  ...
  try {
    ...
    const anySuceeded =
      feodo.ok || urlhaus.ok || c2intel.ok || otx.ok || abuseipdb.ok;
    if (!anySuceeded) {
    ...
    const partial =
      !feodo.ok ||
      enabledButFailed(urlhaus) ||
      !c2intel.ok ||
      enabledButFailed(otx) ||
      enabledButFailed(abuseipdb);
