...
export default async function handler(request) {
  ...
  if (!index) {
    return new Response(
      JSON.stringify({
        error: "No stock index for country",
        code,
        available: false,
      }),
      {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "Upstream error", available: false }),
        {
          status: 502,
          headers: {
            "Content-Type": "application/json",
    ...
    if (!result) {
      return new Response(
        JSON.stringify({ error: "No data", available: false }),
        {
          status: 200,
          headers: {
            "Content-Type": "application/json",
    ...
    const allCloses = result.indicators?.quote?.[0]?.close?.filter(
      (v) => v != null,
    );
    if (!allCloses || allCloses.length < 2) {
      return new Response(
        JSON.stringify({ error: "Insufficient data", available: false }),
        {
          status: 200,
          headers: {
            "Content-Type": "application/json",
    ...
    return new Response(
      JSON.stringify({ error: "Internal error", available: false }),
      {
        status: 500,
        headers: {
          "Content-Type": "application/json",
