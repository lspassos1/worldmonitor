...
export default async function handler(request) {
  ...
  try {
    ...
    if (!country || !code) {
      return new Response(
        JSON.stringify({ error: "country and code required" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        },
    ...
    const contextHash = context
      ? hashString(JSON.stringify(context)).slice(0, 8)
      : "no-ctx";
    ...
    if (context?.score != null) {
      const changeStr = context.change24h
        ...
        : "";
      dataLines.push(
        `Instability Score: ${context.score}/100 (${context.level || "unknown"}) — trend: ${context.trend || "unknown"}${changeStr}`,
      );
    }
    if (context?.components) {
      const c = context.components;
      dataLines.push(
        `Score Components: Unrest ${c.unrest ?? "?"}/100, Security ${c.security ?? "?"}/100, Information ${c.information ?? "?"}/100`,
      );
    }
    if (context?.protests != null)
      dataLines.push(
        `Active protests in/near country (7d): ${context.protests}`,
      );
    if (context?.militaryFlights != null)
      dataLines.push(
        `Military aircraft detected in/near country: ${context.militaryFlights}`,
      );
    if (context?.militaryVessels != null)
      dataLines.push(
        `Military vessels detected in/near country: ${context.militaryVessels}`,
      );
    if (context?.outages != null)
      dataLines.push(`Internet outages: ${context.outages}`);
    if (context?.earthquakes != null)
      dataLines.push(`Recent earthquakes: ${context.earthquakes}`);
    if (context?.stockIndex)
      dataLines.push(`Stock Market Index: ${context.stockIndex}`);
    ...
    if (context?.headlines?.length > 0) {
      ...
      context.headlines
        .slice(0, 15)
        .forEach((h, i) => dataLines.push(`${i + 1}. ${h}`));
    }
