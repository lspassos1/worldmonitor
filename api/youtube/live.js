...
export default async function handler(request) {
  ...
  try {
    ...
    if (videoIdMatch && isLiveMatch) {
      return new Response(
        JSON.stringify({ videoId: videoIdMatch[1], isLive: true }),
        {
          status: 200,
          headers: {
            "Content-Type": "application/json",
            "Cache-Control":
              "public, max-age=300, s-maxage=300, stale-while-revalidate=60", // Cache for 5 minutes
          },
        },
      );
    }
    ...
    return new Response(
      JSON.stringify({ videoId: null, error: error.message }),
      {
        status: 200,
        headers: { "Content-Type": "application/json" },
      },
