...
function extractVolumes(chart) {
  try {
    const result = chart?.chart?.result?.[0];
    return (
      result?.indicators?.quote?.[0]?.volume?.filter((v) => v != null) || []
    );
  } catch {
    return [];
  }
...
export default async function handler(req) {
  ...
  try {
    ...
    const btcVolumes =
      btcChart.status === "fulfilled" ? extractVolumes(btcChart.value) : [];
    const btcAligned =
    ...
    const result = {
      ...
      signals: {
        ...
        flowStructure: {
          status: flowStatus,
          btcReturn5: btcReturn5 !== null ? +btcReturn5.toFixed(2) : null,
