...
const SERVICES = [
  ...
  },
  {
    ...
    name: "Docker Hub",
    statusPage:
      "https://www.dockerstatus.com/1.0/status/533c6539221ae15e3f000031",
    customParser: "statusio",
    category: "dev",
  },
  {
...
function normalizeStatus(indicator) {
  ...
  if (
    val === "none" ||
    val === "operational" ||
    val.includes("all systems operational")
  ) {
    return "operational";
  }
  ...
  if (
    val === "minor" ||
    val === "degraded_performance" ||
    val === "partial_outage" ||
    val.includes("degraded")
  ) {
    return "degraded";
  }
  ...
  try {
    ...
    if (service.customParser === "gcp") {
      ...
      const activeIncidents = Array.isArray(data)
        ? data.filter(
            (i) => i.end === undefined || new Date(i.end) > new Date(),
          )
        : [];
      ...
      const severity = activeIncidents.some((i) => i.severity === "high")
        ? "outage"
        : "degraded";
      return {
        ...service,
    ...
    if (service.customParser === "instatus") {
      ...
      if (pageStatus === "UP") {
        ...
        return {
          ...service,
          status: "unknown",
          description: pageStatus || "Unknown",
        };
      }
    }
    ...
    if (service.customParser === "statusio") {
      ...
      return {
        ...service,
        status: "unknown",
        description: overall?.status || "Unknown status",
      };
    }
    ...
    if (service.customParser === "incidentio") {
      ...
      if (text.startsWith("<!") || text.startsWith("<html")) {
        ...
        const operationalMatch = text.match(
          /All Systems Operational|fully operational|no issues/i,
        );
        if (operationalMatch) {
          return {
            ...service,
        ...
        const degradedMatch = text.match(
          /degraded|partial outage|experiencing issues/i,
        );
        if (degradedMatch) {
          return {
            ...service,
        ...
        return {
          ...service,
          status: "unknown",
          description: "Could not parse status",
        };
      }
      // Parse JSON response
      try {
        ...
        if (
          indicator === "none" ||
          description.toLowerCase().includes("operational")
        ) {
          return {
            ...service,
            status: "operational",
    ...
    if (text.startsWith("<!") || text.startsWith("<html")) {
      return {
        ...service,
        status: "unknown",
        description: "Blocked by service",
      };
    }
    ...
    if (data.status?.indicator !== undefined) {
      ...
      status = normalizeStatus(
        data.status.indicator || data.status.description,
      );
  ...
  return new Response(
    ...
    }),
    {
      headers: {
        ...
        ...cors,
        "Cache-Control":
          "public, max-age=60, s-maxage=60, stale-while-revalidate=30", // 1 min cache
      },
    },
  );
}
