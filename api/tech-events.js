...
const CURATED_EVENTS = [
  {
    ...
    source: "curated",
    description:
      "Intelligence Everywhere: The AI Economy - 8,000+ attendees, 400+ startups",
  },
  {
...
function parseDevEventsRSS(rssText) {
  ...
  for (const match of itemMatches) {
    ...
    const coords =
      location && location !== "Online" ? normalizeLocation(location) : null;
    if (location === "Online") {
      // Mark as virtual
    ...
    events.push({
      ...
      coords:
        coords ||
        (location === "Online" ? { virtual: true, original: "Online" } : null),
      startDate: startDate,
...
export default async function handler(req) {
  ...
  try {
    ...
    const mappableCount = conferences.filter(
      (e) => e.coords && !e.coords.virtual,
    ).length;
