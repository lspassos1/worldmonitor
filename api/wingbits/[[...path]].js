...
export default async function handler(req) {
    ...
    try {
      ...
      return Response.json(data, {
        headers: {
          ...corsHeaders,
          "Cache-Control":
            "public, max-age=86400, s-maxage=86400, stale-while-revalidate=3600", // 24h - aircraft details rarely change
        },
      });
    } catch (error) {
      return Response.json(
  ...
  if (path === "/flights" && req.method === "GET") {
    try {
      ...
      if (!la || !lo) {
        return Response.json(
          { error: "lat (la) and lon (lo) required" },
          {
            status: 400,
            headers: corsHeaders,
          },
      ...
      return Response.json(data, {
        headers: {
          ...corsHeaders,
          "Cache-Control":
            "public, max-age=30, s-maxage=30, stale-while-revalidate=15", // 30 seconds - live data
        },
      });
    } catch (error) {
